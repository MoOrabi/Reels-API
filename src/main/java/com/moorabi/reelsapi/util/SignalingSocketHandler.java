...
public class SignalingSocketHandler extends TextWebSocketHandler {
    ...
    public void afterConnectionEstablished(WebSocketSession session) throws Exception {
        ...
        connectedUsers.values().forEach(webSocketSession -> {
            try {
                webSocketSession.sendMessage(new TextMessage(WebRTCUtil.getString(newMenOnBoard)));
            } catch (Exception e) {
    ...
    protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
        ...
        SignalMessage signalMessage = WebRTCUtil.getObject(message.getPayload());
        ...
        if (destSocket != null && destSocket.isOpen()) {
            ...
            final String resendingMessage = WebRTCUtil.getString(signalMessage);
    ...
    private void removeUserAndSendLogout(final String sessionId) {
        ...
        connectedUsers.values().forEach(webSocket -> {
            try {
                webSocket.sendMessage(new TextMessage(WebRTCUtil.getString(menOut)));
            } catch (Exception e) {
                LOG.warn("Error while message sending.", e);
            }
        });
